...
export default function Home() {
  ...
  const checkAuthStatus = async () => {
    ...
    try {
      ...
      // console.log("Response status from /api/user:", response.status);
      if (response.ok) {
        ...
        // console.log("Failed to fetch user, status:", response.status, "body:", errorText);
        window.location.href = '/login';
        return;
      }
    } catch (error) {
  ...
  const handleMetricSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (res.status === "success") {
        ...
        setPopup({ message: res.message || "Failed to add metric.", type: "error" });
      }
    } catch (err) {
  ...
  function getWeightSub(weightArr: number[]) {
    if (!weightArr || weightArr.length === 0) return { sub: "No data", color: "#6b7280" };
    if (weightArr.length === 1) return { sub: "No previous data", color: "#6b7280" };
    const diff = weightArr[weightArr.length - 1] - weightArr[weightArr.length - 2];
    if (diff < 0) return { sub: `Lost ${Math.abs(diff).toFixed(1)} kg`, color: "#ef4444" };
    if (diff > 0) return { sub: `Gained ${diff.toFixed(1)} kg`, color: "#10b981" };
  ...
  function getHeartStatus(systolicArr = [], diastolicArr = []) {
    ...
    if (systolic < 120 && diastolic < 80) {
      return { status: "Normal", description: "Healthy blood pressure, low risk", color: "#10b981" };
    }
    if (systolic >= 120 && systolic <= 129 && diastolic < 80) {
      return { status: "Warning (Elevated)", description: "Slightly increased risk, lifestyle changes advised", color: "#f59e42" };
    }
    if ((systolic >= 130 && systolic <= 139) || (diastolic >= 80 && diastolic <= 89)) {
      return { status: "Danger (Mild Hypertension)", description: "Early high blood pressure, requires monitoring and possible treatment", color: "#f59e42" };
    }
    if (systolic >= 140 || diastolic >= 90) {
      if (systolic > 180 || diastolic > 120) {
        return { status: "Emergency (Crisis)", description: "Immediate medical attention needed", color: "#ef4444" };
      }
      return { status: "Danger (Moderate/Severe Hypertension)", description: "High risk, needs medical treatment", color: "#ef4444" };
    }
    return { status: "Unknown", description: "No data available", color: "#6b7280" };
  }
      ...
      <div className="min-h-screen w-full bg-gradient-to-br from-[#eaf1fb] to-[#e3e6ef] flex flex-col items-center justify-center p-2 sm:p-10 mt-29">
        ...
        <div className="flex flex-col gap-4 w-full max-w-md mx-auto sm:hidden">
            ...
            <div className="flex flex-row justify-between items-start h-full">
              <div className="flex flex-col justify-between h-full">
                <div>
                  ...
                  <span className="text-xs font-semibold block mt-1" style={{ color: getWeightSub(metrics?.weight || []).color }}>
                    {getWeightSub(metrics?.weight || []).sub}
                  </span>
                </div>
                <div className="mt-auto flex items-end gap-1">
                  <span className="text-2xl font-bold text-blue-700">{metrics?.weight?.length ? metrics.weight[metrics.weight.length - 1].toFixed(1) : "--"}</span>
                ...
                <div>
                  ...
                  <span className="text-xs text-gray-400 block mt-1">
                    {metrics?.food?.length && goals.calories
                      ? `${metrics.food[metrics.food.length - 1]}/${goals.calories} Kcal`
                      : `--/${goals.calories || '--'} Kcal`}
                  </span>
                </div>
                <div className="mt-auto flex items-end gap-1">
            ...
            <div className="relative w-full max-w-[300px] h-[180px] flex items-center justify-center">
              ...
              <div className="flex flex-col items-center -mx-8 -mt-32">
                ...
                <div className="flex flex-col items-center -mx-8 mt-42">
                  <span className="text-xs text-gray-800 mt-2">{goals.steps ? `${goals.steps.toLocaleString()} Steps` : '-- Steps'}</span>
                </div>
          ...
          <div className="relative rounded-2xl bg-white/70 shadow-xl p-4 flex flex-col items-center w-full border border-gray-200 h-48">
            ...
            {(() => {
              ...
              return (
                <>
                  <div className="absolute top-4 right-4 cursor-pointer" onClick={() => setHeartFlipped(f => !f)}>
                    <svg style={{ transform: heartFlipped ? 'scaleX(-1)' : 'none', transition: 'transform 0.3s' }} width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#bdbdbd" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
                  </div>
                  {!heartFlipped ? (
                    <>
                      <div className="w-full h-32">
                        <Image
                          ...
                          style={{ position: "absolute", left: "33%", transform: "translateX(-50%)", top: "40px" }}
                        />
                      </div>
            <div className="mt-auto w-full">
              ...
              <div className="flex items-center gap-2 mt-1">
                    ...
                    <div className="flex flex-1 flex-col justify-center items-center w-full h-full">
                      ...
                      <span className="text-xs text-center mt-2 text-gray-700">{description}</span>
                    </div>
                  )}
                </>
              );
            ...
            <div className="flex items-end justify-between">
              ...
              <div className="flex flex-col items-end">
                <span className="text-lg font-bold text-blue-700">
                  {metrics?.blood_pressure?.systolic?.length && metrics?.blood_pressure?.diastolic?.length
        ...
        <div className="hidden sm:grid w-full max-w-7xl grid-cols-[385px_480px_260px] grid-rows-[120px_340px] gap-8 bg-white/0">
          ...
          <div className="col-start-3 col-end-4 row-start-1 w-[240px] h-[450px] row-end-3 rounded-2xl bg-white/60 shadow-xl flex items-center justify-center">
            <Image src="/images/output.png" alt="Body" width={260} height={220} className="w-full max-w-[290px]" />
          </div>
            ...
            <div className="relative rounded-2xl bg-white/70 shadow-xl p-6 flex flex-col items-center h-[300px] min-w-[220px] border border-gray-200 transition-transform duration-500">
              ...
              {!heartFlipped ? (
                <>
                  ...
                  <Image src="/images/heart.png" alt="Heart" width={170} height={70} className="mb-4 heart-beat" />
              <div className="mt-auto w-full">
                <div className="text-lg font-semibold text-gray-800">Heart</div>
                <div className="flex items-center gap-2 mt-1">
                      <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: getHeartStatus(metrics?.blood_pressure?.systolic, metrics?.blood_pressure?.diastolic).color }} />
                      <span className="text-sm" style={{ color: getHeartStatus(metrics?.blood_pressure?.systolic, metrics?.blood_pressure?.diastolic).color }}>{getHeartStatus(metrics?.blood_pressure?.systolic, metrics?.blood_pressure?.diastolic).status}</span>
                </div>
              </div>
                </>
              ) : (
                <div className="flex flex-1 flex-col justify-center items-center w-full h-full">
                  <span className="text-lg font-bold" style={{ color: getHeartStatus(metrics?.blood_pressure?.systolic, metrics?.blood_pressure?.diastolic).color }}>{getHeartStatus(metrics?.blood_pressure?.systolic, metrics?.blood_pressure?.diastolic).status}</span>
                  <span className="text-sm text-center mt-2 text-gray-700">{getHeartStatus(metrics?.blood_pressure?.systolic, metrics?.blood_pressure?.diastolic).description}</span>
                </div>
              )}
            </div>
            {/* Lungs Box */}
          ...
          <div className="flex-1 flex flex-col w-[650px] h-[340px] gap-6 -ml-100">
            ...
            <div className="grid grid-cols-3 gap-6">
                  ...
                  <div className="flex flex-col items-end ml-31 ">
                    {metrics?.blood_pressure?.systolic?.length && metrics?.blood_pressure?.diastolic?.length ? (
                      <div className="flex flex-col items-end leading-tight">
                        <span className="font-semibold text-blue-700" style={{ fontSize: "18px" }}>
                          {metrics.blood_pressure.systolic[metrics.blood_pressure.systolic.length - 1]}
                        </span>
                        <span className="text-xs text-gray-900 font-bold">/</span>
                        <span className="font-semibold text-blue-700" style={{ fontSize: "18px" }}>
                          {metrics.blood_pressure.diastolic[metrics.blood_pressure.diastolic.length - 1]}
                        </span>
                      </div>
                    ) : (
              ...
              <div className="relative rounded-2xl bg-white/60 border border-gray-200 p-6 flex flex-col justify-between min-h-[120px]">
                <div className="flex items-center gap-2 mb-2">
                  <Image src="/images/heart.png" alt="Heart" className="-mt-4" width={18} height={18} />
                  <span className="text-sm font-medium text-gray-700 -mt-4">Heart Rate</span>
                </div>
                <div className="flex items-end justify-between mt-2">
                  {/* Placeholder for chart */}
                  {/* Heart Rate chart absolutely positioned at bottom left edge */}
      ...
      {showModal && (
        ...
        >
          ...
          >
            ...
            <form id="metricForm" className="flex flex-col gap-3" onSubmit={handleMetricSubmit}>
